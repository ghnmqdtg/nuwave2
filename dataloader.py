...
def create_vctk_dataloader(hparams, cv, sr=24000):
    ...
    if cv == 0:
        ...
        return DataLoader(
            dataset=(
                VCTKMultiSpkDataset(hparams, cv)
                if cv == 1
                else VCTKMultiSpkDataset(hparams, cv, sr)
            ),
            collate_fn=collate_fn,
...
class VCTKMultiSpkDataset(Dataset):
    ...
    def __getitem__(self, index):
        wav, _ = rosa.load(self.data_list[index], sr=self.hparams.audio.sampling_rate)
        ...
        if len(wav_l) < len(wav):
            wav_l = np.pad(
                wav, (0, len(wav) - len(wav_l)), "constant", constant_values=0
            )
